...
import { type Cache, type CacheEntry, provideCache } from "vite-plugin-react-use-cache/runtime";
  ...
  const cache = {
    ...
    async revalidateTag(
      ...
      for (const key of keys ?? []) {
        ...
        if (!stored) continue;
        await storage.remove(`item-${key}`);
        await Promise.all(stored.tags.map((t) => (this.revalidateTag as any)(t, seen)));
      }
      return seen.tags;
    },
